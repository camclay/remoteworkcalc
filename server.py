...
def create():
  ...
  #return render_template('index.html')
    ...
    totalSavings = round(totalCommuteCost + (hourly * timeSaved))
    annualRaise = int(yearlyCommuteCost + (hourly * (commuteRoundTripHrs * avgWorkingDays)))
    ...
    "This saved you approximately <b>${}</b> on your monthly commute expenses.<br>" \
    "Working remote is equal to a <b>${}</b> annual raise.<br>" \
    "You'd need to be compensated <b>${}</b> to return to the office." \
    .format(totalBusDays,timeSaved,totalCommuteCost,annualRaise,totalSavings) 
    return respStr
